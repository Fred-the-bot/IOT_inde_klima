...
reset_button = Pin(13)
adc = ADC(Pin(36)) #A4 på esp32 til mikrofonen
adc.read()
d = dht.DHT11(Pin(4))
...
#ntptime.settime() denne sætter tiden korrekt efter reset (da de kan resette tiden) pt. udkommenteret da den ikke kommer på WAN, men istedet i lukket lan.
...
while (i<5):
     ...
     if (i==5):
         ...
         temp = d.temperature()
         hum = d.humidity()
         calibration=(0.0596*(value/5)+116.57)/66+57 #kalibreret via 120+ målinger ved ca. 66dB.
         print(calibration," afrundet")
         calibration=math.ceil(calibration) #Runder resultatet til nærmeste integer
         print(calibration, "smidt ind i skema")
         ...
         if station.isconnected():
             green_led.on()
             test = urequests.post(SERVER, headers = {'content-type': 'application/x-www-form-urlencoded'}, data="api_key=" + apiKeyValue + "&sensor=" + sensorName + "&location=" + sensorLocation + "&value1=" + temp + "&value2=" + hum + "&value3=" + lyd) #poster vores tekst i formatet (IP,HEADER,DATA)
